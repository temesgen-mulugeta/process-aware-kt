...
# Train one (arm, seed)
# ---------------------------------------------------------------------------
def train_one(sequences: list[D.Sequence], arm: str, seed: int, log: Logger,
              *, evaluation_sequences=None, prediction_path=None) -> dict:
    C.set_seed(seed)
    ...
    evaluation_sequences = sequences if evaluation_sequences is None else evaluation_sequences
    c_true, c_prob, d_true, d_prob = _collect_probs(net, evaluation_sequences, "test")
    masks = np.concatenate([s.next_assoc[_kind_mask(s, "test")].numpy() for s in evaluation_sequences])
    students = np.concatenate([np.repeat(s.student_id, int(_kind_mask(s, "test").sum())) for s in evaluation_sequences])
    positions = np.concatenate([np.array(s.target_global_idx)[_kind_mask(s, "test").numpy()] for s in evaluation_sequences])
    if prediction_path is not None:
        np.savez_compressed(prediction_path, y=d_true, p=d_prob, assoc=masks,
                            student=students, position=positions, correct_y=c_true,
                            correct_p=c_prob, threshold=thresh)
    ...
    return {"seed": seed, "threshold": thresh, "epochs": epoch + 1,
            "best_val_loss": best_val, "parameters": sum(p.numel() for p in net.parameters()),
            "flat": flat, "per_category_f1": dm["per_category_f1"],
...
def main() -> None:
    ...
    ap.add_argument("--arms", nargs="+", default=list(F.ARMS), choices=list(F.ALL_ARMS))
