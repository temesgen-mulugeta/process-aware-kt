...
We build ONE feature store (a dict keyed by (student_id, seq_pos)) holding, per
...
class SummaryEmbedder:
    ...
    def encode(self, texts: list[str]) -> np.ndarray:
        model = self._ensure()
        # Blank summaries are encoded as a space, also normalized (not zero vectors).
...
def build_feature_store(table: list[dict], embedder: SummaryEmbedder | None = None
    ...
    # Training must never silently call an external analyzer on a cache miss.
    if not llm_analyzer.MOCK:
        missing = [r for r in table if not llm_analyzer.use_cached(r)]
        if missing:
            raise RuntimeError(f"Missing {len(missing)} cached analyses; run offline extraction first")
    ...
    for rec, a, emb in zip(table, analyses, summary_embeds):
        store[(rec["student_id"], rec["seq_pos"])] = StepFeatures(
            id=rec["id"],
...
# assoc_multihot.)
ARMS = ("A", "B", "C")
# Ablations physically remove absent components; dimensions/parameter counts differ.
SIGNALS = {"A": "", "B": "MES", "C": "ME", "B_M": "M", "B_E": "E",
           "B_S": "S", "B_ME": "ME", "B_MS": "MS", "B_ES": "ES"}
ALL_ARMS = tuple(SIGNALS)
USES_ERROR_EMBED = {arm: "E" in signals for arm, signals in SIGNALS.items()}
...
def step_dense_vector(sf: StepFeatures, arm: str) -> np.ndarray:
    ...
    if arm.startswith("B_") and arm in SIGNALS:
        parts = [sf.assoc_multihot, correct]
        if "M" in SIGNALS[arm]:
            parts.append(sf.llm_missing_multihot)
        if "S" in SIGNALS[arm]:
            parts.append(sf.summary_embed)
...
def step_error_id(sf: StepFeatures, arm: str) -> int:
    ...
    if arm in SIGNALS and arm != "C" and "E" in SIGNALS[arm]:
...
def dense_dim(arm: str) -> int:
    ...
    if arm not in SIGNALS:
        raise ValueError(f"unknown arm {arm!r}")
    if arm == "C":
        return base + C.NUM_CONCEPTS + C.SUMMARY_EMBED_DIM
    return base + (C.NUM_CONCEPTS if "M" in SIGNALS[arm] else 0) + (C.SUMMARY_EMBED_DIM if "S" in SIGNALS[arm] else 0)   # + missing(55) + summary(384)
